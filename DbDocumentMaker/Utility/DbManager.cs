...
namespace DbDocumentMaker.Utility
{
    class DbManager
    {
        ...
        {
            const string sheetSystemName = "#system.name";
            const string sheetSystemDescription = "#system.description";
            ...
            var SystemNameLocation = tableSheet.FindCellLocation(sheetSystemName);
            if (SystemNameLocation != null)
            {
                tableSheet.SetFirstMatchCellContentInRow(SystemNameLocation.Value.Y, sheetSystemName, SystemName);
            }
            var SystemDescriptionLocation = tableSheet.FindCellLocation(sheetSystemDescription);
            if (SystemDescriptionLocation != null)
            {
                tableSheet.SetFirstMatchCellContentInRow(SystemDescriptionLocation.Value.Y, sheetSystemDescription,SystemDescription);
            }
            var tableIndex = 1;
